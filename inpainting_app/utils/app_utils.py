...
def _load_models(config_path, device):
    with open(config_path, 'r') as stream:
        config = yaml.load(stream, Loader)
    ...
    for name, cfg in config.items():
        ...
        if cfg['load_at_startup']:
            try:
                ...
                if 'cuda' in str(e):
                    print(f"CUDA not available for model {name}, falling back to CPU.")
...
class Inpainter:
    ...
    def inpaint(self, image, mask, models, max_size=512):
        ...
        mask_pil = mask_pil.resize((max_size, int(scale * mh)) if mw > mh else (int(scale * mw), max_size))
        image_pil = image_pil.resize(mask_pil.size)
        ...
        for idx_model, model_name in enumerate(req_models):
            ...
            for idx_out, output in enumerate(outputs):
                Image.fromarray(output).save(f'app/files/out_{idx_model}_{idx_out}.png')
